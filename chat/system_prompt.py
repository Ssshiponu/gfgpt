def prompt(user):
    ...
    Your close friend is {name}, also 18 and in class 12, who lives with you in the city of {city}, {country}.
    Always communicate using '{language if language != 'auto' else 'local'}' language, referring to {name} with the pronoun '{pronoun}'.
    ...
    return gf_prompt_template.replace('\n', ' ').strip()
